...
def OpenSnd(filename,fs=44100, channels=1):
    try:
        if filename == "":
            ...
            with sf.SoundFile(filename, 'rb') as f:
                data, samplerate = f.read(filename)
...
def plot_spectrogram(msg, cw_msg, spec, sample_rate, L, starts, mappable=None):
    ...
    xstretch = 2000/(URANGE-LRANGE)
    im = ax2.imshow(specshowx, origin='lower', cmap=plt.get_cmap("twilight_r"), aspect=xstretch)
...
def main():
    ...
    try:
        ...
        if msg == 'create':
            ...
            print("nothing entered = restartt program")
            sys.exit()
